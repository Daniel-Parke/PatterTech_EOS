#!/usr/bin/env python3
"""run.py must boot and answer HTTP.
...
def scratch_dir():
    if len(sys.argv) < 2:
        emit(False, "usage: c1_server_starts.py <scratch-dir>")
    ...
    try:
        if not up:
            ...
            if proc.poll() is not None:
                ...
                detail = lines[-1] if lines else "process exited silently"
            emit(False, "server did not come up on 127.0.0.1:8765: %s"
                 % detail)
        emit(True, "server boots and answers HTTP on 127.0.0.1:8765")
    finally:
        _stop(proc)
