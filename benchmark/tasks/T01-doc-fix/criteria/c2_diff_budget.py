#!/usr/bin/env python3
"""Total diff against the baseline root commit is small.
...
def scratch_dir():
    if len(sys.argv) < 2:
        emit(False, "usage: c2_diff_budget.py <scratch-dir>")
...
def main():
    ...
    total = 0
    numstat = _git(scratch, "diff", "--numstat", root)
    ...
    for rel in _untracked_files(scratch):
        path = scratch / rel
        if path.is_file():
            text = path.read_text(encoding="utf-8", errors="replace")
            total += len(text.splitlines())
    if total > LIMIT:
        emit(False, "diff is %d lines against a budget of %d" % (total, LIMIT))
    emit(True, "diff is %d lines, within the %d line budget" % (total, LIMIT))
