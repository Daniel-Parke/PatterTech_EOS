---
summary: The FieldKit constitution, Part I product doctrine, Parts II and III the protected law
type: template
tags: [eos]
compiled_from: kernel/templates/org/CONSTITUTION.tpl.md
---

# The FieldKit Constitution
...
FieldKit exists so the firm stops losing survey sheets. Four rules
bind the product:

1. **Speed in the field wins.** Recording a survey on a phone must be
   quicker than filling in the paper sheet. Anything that slows the
   surveyor down is a defect, whatever else it buys, because the
   operator's own verdict is that the surveyors will not use it
   otherwise.
2. **The record is central and it is the firm's.** Every survey lands
   in one place, attributed to the signed-in surveyor who recorded
   it, and leaves clean as CSV whenever the office asks. Data the
   firm cannot get out is data the firm does not own.
3. **Structured the way the firm already surveys.** This is one
   firm's site surveys, not a general form builder. The survey shape
   follows the firm's existing practice and changes only when the
   practice does.
4. **A survey record is append-only.** A correction is a new version,
   never an edit in place. A survey that quietly changed is as bad as
   a survey that was lost, and losing surveys is the whole reason
   this exists.

Out of scope at v1, deliberately: payments, a client portal, a native
app, photo uploads. Text and numbers first.
...
*Adopted 2026-08-03 (Session 0). No amendments yet.*
