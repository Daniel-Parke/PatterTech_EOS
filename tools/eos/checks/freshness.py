...
def check_f003_bulk_review_dates(ctx: dict) -> list:
    ...
    model: RepoModel = ctx["model"]
    groups: dict = {}
    cohort_dates: dict[str, set[str]] = {}
    cohort_paths: dict[str, set[str]] = {}
    out = []
    for rec in model.files:
        if not _in_scope(rec):
            continue
        if rec.fm.data.get("derived"):
            # A derived view inherits its source review schedule and is
            # regenerated by the checker. Counting it again can turn one
            # truthful review into a false bulk-set cohort.
            continue
        cohort = rec.path
        declared_cohort = rec.fm.data.get("review_cohort")
        if declared_cohort is not None:
            if not isinstance(declared_cohort, str) or not declared_cohort.strip():
                out.append(Finding(
                    "F003", "error", rec.path,
                    "review_cohort must be a non-empty string",
                ))
            else:
                slug = declared_cohort.strip()
                cohort = f"review-cohort:{slug}"
                cohort_dates.setdefault(slug, set())
                cohort_paths.setdefault(slug, set()).add(rec.path)
        ...
        for key in ("review_by", "review"):
            ...
            if isinstance(value, str) and MONTH_RE.match(value.strip()):
                month = value.strip()
                groups.setdefault(month, {})[cohort] = rec.path
                if isinstance(declared_cohort, str) and declared_cohort.strip():
                    cohort_dates[declared_cohort.strip()].add(month)
    for slug in sorted(cohort_paths):
        paths = sorted(cohort_paths[slug])
        dates = sorted(cohort_dates[slug])
        if len(paths) < 2:
            out.append(Finding(
                "F003", "error", paths[0],
                f"review cohort {slug} has one file; schedule it independently",
            ))
        if len(dates) != 1:
            detail = ", ".join(dates) if dates else "no YYYY-MM date"
            out.append(Finding(
                "F003", "error", paths[0],
                f"review cohort {slug} must share one YYYY-MM date, found {detail}",
            ))
