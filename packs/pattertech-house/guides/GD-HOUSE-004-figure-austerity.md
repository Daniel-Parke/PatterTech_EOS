...
- **PatterTech long reads (2026-07, inherited)**: the diagram kit gives
  every figure the same annotation voice, and the plate number joined to
  its caption by a hairline is the device that runs through all of them.
  The one promoted figure per piece is usually the opening schematic,
  which may animate to show the system working and freezes under reduced
  motion.
- **The one number an argument turns on (2026-07, inherited)**: set
  monumentally between hairlines in the reading column, counting up and
  glinting once on arrival. A row of numbers is a plaque and gets none of
  that. This is B applied to a value rather than a chart.
