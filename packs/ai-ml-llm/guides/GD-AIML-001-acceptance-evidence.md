...
Assume `B. Model judge over a rubric` was selected and the outcome failed. Test this option's stated failure mechanism first: the judge is an instrument with its own error, and it is worthless until validated against human labels. See `packs/ai-ml-llm/guides/GD-AIML-003-who-grades-the-output.md`.
