---
summary: Building on language models, result identity, private held-out sets, pinned model ids, validated judges and a person in front of consequential output
type: playbook
...
Five requirements bind. A run that breaks one fails, whatever the demo
looked like. The authority audit under ADR-0008 moved two of the
original seven to defaults: the eval-run rule is now D9 and the
abstention rule is now D10. The five that stayed keep their numbers, so
the citations in the guides, refs and exemplar still resolve, which is
why the list below starts at B2.
...
it.** The venture holds an acceptance set the providers have never seen,
...
call site.** No "latest", no bare family name. Pinned is a property, not
...
**B5. A judge is validated against human labels before its score decides
anything.** Where a model grades output and the score selects between
...
**B7. Consequential model output is reviewed by a person before it takes
effect.** Where output moves money, changes access, contacts a customer,
...
**D4. Lexical retrieval first, embeddings earn their place.** Start with
...
**D5. Split retrieval metrics by stage.** Measure context precision and
...
**D7. Order every prompt stable prefix first, variable suffix last, and
assert the cache hit rate.** Cached prefix reads are charged at a tenth
...
**D9. Run a machine-repeatable eval before a model-facing change
ships.** A prompt, model, retriever or decoding change goes through a
...
**D10. Score abstention, and report the abstention rate beside
accuracy.** The output contract of a model-backed component includes a
...
- **Compiled prompt optimisation against hand-written legible context.**
...
- **Which evaluation framework, and where the eval lives.** The
  ...
  defensible as long as the acceptance set is versioned with the code it
  gates.
