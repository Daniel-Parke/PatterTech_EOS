...
# WG-UIUX-003: Which design philosophy does this surface take?
...
None, deliberately. This Wargame has no house default and adding one
...
Facts that change the engagement answers above can overturn the safe default. Test ****Can the user leave?** A statutory service, an internal tool and a paid-acquisition landing page differ mainly here.** and ****What is the surface for?** Completing a task, reading, monitoring, browsing, or converting.** against the selected option. A contrary result counts only when it uses the same representative constraints and changes the decision rule, rather than merely preferring another style.
