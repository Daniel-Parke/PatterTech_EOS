...
# WG-UIUX-006: Where do tokens live and how do they reach each platform?
...
design tools. PACK.md B6 already binds define-once-and-generate; this Wargame decides where the once is.
...
Facts that change the engagement answers above can overturn the safe default. Test ****How many platforms consume the values.** One web app has a much weaker case for a build step than web plus two native apps.** and ****Who edits values.** Designers in a tool, or engineers in a repo.** against the selected option. A contrary result counts only when it uses the same representative constraints and changes the decision rule, rather than merely preferring another style.
