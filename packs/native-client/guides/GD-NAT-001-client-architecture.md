...
- **Accessibility obligation.** Platform semantics APIs are richest
  where you use the platform's own toolkit (FRAG-NATIVE-18).
...
semantics APIs are richest here (FRAG-NATIVE-18). Buys the best
...
and independent grade (FRAG-NATIVE-17). Buys one implementation of the
...
(FRAG-NATIVE-16). Buys one surface and one team. Costs platform
...
opt-out without declaring the old path dead (FRAG-NATIVE-15). Read that
...
outright under review rule 4.2 (FRAG-NATIVE-03).
...
- **Grading as disclosure (external, inherited)**: the per-target
  stability grade (FRAG-NATIVE-17) is what makes a cross-platform bet
