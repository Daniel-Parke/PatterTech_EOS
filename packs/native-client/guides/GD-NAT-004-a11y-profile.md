...
(FRAG-NATIVE-01). The reviewable artefact becomes the semantics
...
labels and null descriptions for decoration (FRAG-NATIVE-18). And the
...
technology, and support for user preference settings (FRAG-NATIVE-02).
...
minimum and no contrast ratio at all (FRAG-NATIVE-18).
...
audit type, failing the build on any violation (FRAG-NATIVE-19). Buys a
...
under clause 11 (FRAG-NATIVE-02). Buys coverage of the defect class no
...
ahead of the binding standard rather than behind it (FRAG-NATIVE-02).
...
figure is published for it at all (FRAG-NATIVE-19), and the equivalent
...
- **The mapping has no legal force (external, inherited)**: the mobile
  ...
  Level A and AA only (FRAG-NATIVE-01). Use it to translate. Do not
