...
GD-NAT-001, ruled B: shared logic with a native user interface. Two
...
B1. Classified using `packs/native-client/refs/WRITE_CLASSES.md`:
...
Only then was the sync approach chosen: GD-NAT-002 option D, read-path
...
GD-NAT-004, ruled C. Six screens, so six claims, not one.
...
GD-NAT-003, ruled C. `RELEASE.md` has two sections, one per store,
