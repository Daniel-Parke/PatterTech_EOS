...
# WG-NAT-004: How much non-web accessibility assurance?
...
- **How much custom drawing there is.** A screen of platform controls
  ...
  especially under architecture C in WG-NAT-001.
...
Facts that change the engagement answers above can overturn the safe default. Test ****Statutory duty.** European Accessibility Act reach, public sector procurement, or a regulated market.** and ****Whether the user can go elsewhere.** Exclusion only costs where there is no alternative route.** against the selected option. A contrary result counts only when it uses the same representative constraints and changes the decision rule, rather than merely preferring another style.
