...
Asked for by B5, a default since the 2026-08 audit, whenever a design
uses anything above direct single-agent. One file, under 120 lines,
front-matter with `summary`, `type` and `tags` including `eos`. Six
level-two sections, in this order.

## Topology

Name the topology for each stage of the work, using the exact strings
from `packs/agentic-development/references/TOPOLOGY_CARD.md`. One per stage,
