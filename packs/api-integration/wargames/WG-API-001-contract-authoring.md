...
# WG-API-001: who authors the contract, and when?
...
Facts that change the engagement answers above can overturn the safe default. Test **Who reads the contract before the code exists: another team, an external consumer, or nobody?** and **Is the bigger risk a badly designed boundary, or a document that quietly stops matching the service?** against the selected option. A contrary result counts only when it uses the same representative constraints and changes the decision rule, rather than merely preferring another style.
