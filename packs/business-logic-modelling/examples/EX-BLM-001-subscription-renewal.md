...
Wargame WG-BLM-001. The rules are thin: a price, a period, a status and
...
the field set from
`packs/business-logic-modelling/references/BOUNDARY_WRITE_UP.md`. Everything
else stays a function.

## Step 3: money

Requirement B1 and Wargame WG-BLM-003. The float goes. Money becomes an
...
Requirement B2 and Wargame WG-BLM-004. The naive local datetime goes. The
...
`packs/business-logic-modelling/references/TIME_TYPES.md`.
...
Default D3 and Wargame WG-BLM-002. The three booleans go. Trialling,
...
library and no engine here. That is the threshold in WG-BLM-002, not a
preference about dependencies.

## Step 6: the price bands

Five price bands that marketing changes on its own clock, previously a
chain of conditionals edited by a developer every time. Wargame
WG-BLM-002 rules B: a flat decision table with declared inputs,
...
Default D10 and Wargame WG-BLM-005. The renewal event was previously
