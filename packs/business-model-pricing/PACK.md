...
**Paths.** Anything under a pricing, plans, billing, checkout, subscribe,
tariff, invoicing, finance or commercial directory. Price and plan
configuration in payment-provider code. A public pricing or terms page.
Cohort, revenue and retention reporting code.
...
Six requirements bind. Each is law or an accounting standard at the
...
commercial commentary putting it near spring 2027. Build to the
principles; do not hard-code section detail into product copy.

**B3. Payment terms are written down, because they exist either way.**
...
606. Name your framework in the decision record.
...
**B6. No pattern from the regulator's almost-always-harmful list.**
...
abandoned wrongly. A trial length that ships without a test plan is a
number the evidence does not support. See
...
`packs/business-model-pricing/research/sources.fragment.json`, each
carrying version, licence, access date and review trigger, pending
integrator import into `registry/evidence.json`. The seven EV ids cited
above already resolve in that ledger. The synthesis is in
`packs/business-model-pricing/research/NOTES.md`.
