---
summary: Validate a token's signature, issuer, audience and expiry on every request, using an algorithm fixed by the verifier.
type: doctrine
tags: [eos]
id: DOC-IDENT-003
statement: Validate a token's signature, issuer, audience and expiry on every request, using an algorithm fixed by the verifier.
kind: doctrine
authority: binding
basis: standard
