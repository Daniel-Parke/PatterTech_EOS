...
The obvious implementation is a new role, `report_viewer`. GD-IDENT-001
...
in a trench coat, and it works, which matters. The guide's decision rule
...
Recorded in the lock-book as a ruling on GD-IDENT-001, with that
...
`packs/identity-access/refs/decision-point-placement.md` argues for: in
...
`packs/identity-access/refs/tenant-isolation-mechanics.md` runs, and two
...
property 2 in `packs/identity-access/refs/break-glass.md` rules out:
...
first-party, so GD-IDENT-002 option A already applies and nothing
...
The one judgement call was step 1, and it went against the guide's first
