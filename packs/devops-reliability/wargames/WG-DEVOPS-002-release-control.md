...
# WG-DEVOPS-002: How does a change reach users once it is deployed?
...
puts you back in WG-DEVOPS-001 option B.

Every flag created under D carries a non-empty owner and an expiry date
in the future. See
`packs/devops-reliability/references/FLAG_AND_ROLLOUT_LIFECYCLE.md`.
...
Facts that change the engagement answers above can overturn the safe default. Test **Blast radius. A change that can only be wrong for one tenant is not the same as one that can corrupt every row.** and **Whether a traffic router and a metrics backend already exist. A promotion query needs something to query.** against the selected option. A contrary result counts only when it uses the same representative constraints and changes the decision rule, rather than merely preferring another style.
