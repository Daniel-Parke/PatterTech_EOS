...
# WG-SUPPLY-004: do we vendor it or depend on it?
...
whatever schedule WG-SUPPLY-003 set.
...
Assume `A. Depend, pinned` was selected and the outcome failed. Test this option's stated failure mechanism first: a build-time dependency on the registry, and it means a compromised release upstream reaches you on whatever schedule WG-SUPPLY-003 set.
...
**Exit condition:** Stop or roll back the selected branch when a build-time dependency on the registry, and it means a compromised release upstream reaches you on whatever schedule WG-SUPPLY-003 set, or when its stated preconditions cease to hold.
...
There is no measurement here. This Wargame rests on toolchain
