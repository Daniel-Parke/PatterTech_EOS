---
summary: The all-in-one guide to the EOS, the Venture A genesis and the development lifecycle
type: guide
tags: [eos]
review_by: 2027-01
status: archived
superseded_by: TOUR.md
---
...
1), how a real venture called Venture A was born from it (Section 2),
and how you build a product inside it day to day (Section 3). Venture A
...
**Section 2 · The Venture A seed pack and the genesis run**
- [Chapter 9 · Where Venture A came from](#chapter-9--where-venture-a-came-from)
...
- [Chapter 17 · The road ahead for Venture A](#chapter-17--the-road-ahead-for-venture-a)
...
tool. Two exist in the estate today. **Venture A** is a full product
...
| `examples/` | Worked instantiations to learn from | `examples/venture-a-seed.md` |
...
- **L** is about twenty base files plus the directories Genesis fills:
  ...
  and knowledge shelves. Venture A is L.
...
execution half before you meet Venture A.
...
- **Phase B** extracted the kernel templates from Venture A's own seed
  pack (Venture A is the kernel's ancestor; more on that in Chapter 9).
- **Phase C** wrote the compile rules and the voice module.
- **Phase D** reseeded Venture A and harvested the lessons.
...
This section follows one venture, Venture A, from an empty organisation
...
Venture A lives in its own repository; this guide describes what
...
condensed worked example also lives at `examples/venture-a-seed.md`.

## Chapter 9 · Where Venture A came from

**What you will learn:** the unusual fact that makes Venture A the ideal
teaching case, and the shape of the venture itself.

Venture A is a lifecycle integrity platform for renewable energy assets.
...
The unusual fact is this. Venture A was seeded, by hand, before the EOS
v1.0 kernel existed. When the framework was generalised, Venture A's own
organisational files were the raw material the kernel templates were
extracted from. So Venture A is the kernel's ancestor. Reseeding it from
...
Venture A runs under a genuine constraint that shapes everything: a
...
step, using Venture A's real rulings. The procedure is
`inception/INCEPTION.md`; the interview protocol is
`inception/INTERVIEW.md`; the wargame walk is
`inception/WALK_ORDER.md`.

A Session 0 runs five phases. Here they are, with what Venture A did in
each.
...
1. **Restate and be corrected.** The agent restates the venture; the
   operator corrects until it is right. Venture A's restatement stood
...
2. **Name the three cheapest deaths.** Not the most dramatic failure
   modes, the cheapest. Venture A's were recorded verbatim: miss the
...
3. **Propose the strictly smaller version.** The agent argues for a
   ...
   own words. Venture A's smaller version was superseded by the Heads of
   Terms, which fenced Oversight into the MVP.

### Phase B: scale and shape

The operator and agent walk `WG-EOS-001` and rule the scale. Venture A
...
lock-book. Venture A walked seventeen wargames and recorded seventeen
...
- `WG-WEB-001` was **argued to B**, against the framework's dark-first
  default. The reasoning: Venture A is a print-native institutional
...
- Eight wargames were **inherited**: their triggers were silent for
  Venture A, so it took the default without ceremony, and those rulings
  count for nothing in promotion. That is correct and honest.

> [!TIP]
> The argued-versus-inherited split is easiest to grasp here. Nine
> times Venture A had a real reason and reasoned to it; eight times it
...
Venture A, and two real nuances that show the discipline under pressure.
The artefact is `docs/COMPILE_REPORT.md` in the Venture A repo.
...
its source. Venture A's has 23 rows. The categories tell the story:
...
- The constitution's Part I (the product doctrine) was carried
  **verbatim** from Venture A's adopted original, because Part I is the
...
and Venture A was a signed, seeded, single-branch organisation ready to
build.
...
Venture A's `org/product/`, `org/decisions/`, `org/standards/` and
`org/work/`.
...
For Venture A, Genesis produced:
...
Venture A processes regulated UK data, so it carried a compliance
...
the template gained a slot, and the wargame gained Venture A's argued
...
> gets better within a cycle. Venture A changed the kernel twice on its
> first day precisely because someone wrote two sentences down.

Section 2 ended with Venture A buildable. Section 3 builds it.
...
software, one verified change at a time. It follows Venture A's first
...
Venture A's `org/decisions/ADR-0003-local-first-development.md`.

Venture A's production stack is AWS and Clerk, and those credentials
...
watching. The order is Venture A's WO-0001, the local harness itself,
...
## Chapter 17 · The road ahead for Venture A
...
> You copy the L3-WORK launcher from Venture A's `OPERATORS_GUIDE.md`,
...
> [!NOTE]
> **Architecture note.** The four horizons in Venture A's roadmap (MVP,
...
- **§A5 walk-through.** Venture A's acceptance test: the end-to-end
...
| The condensed Venture A example | `examples/venture-a-seed.md` |
| The venture directory | `registry/PROJECTS.md` |

For Venture A's own files (its constitution, domain model, ADRs,
standards, specs, work orders and session logs), open the Venture A
